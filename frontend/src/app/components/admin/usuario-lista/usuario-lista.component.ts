...
export class UsuarioListaComponent implements OnInit {
  ...
  agregarUsuario() {
    this.router.navigate(['/admin/usuario/agregar']);
  }

  ver_perfil_usuario(id:string) {
    this.router.navigate(['/admin/usuario/perfil', encryptNumber(id)]);
  }

  navegar_editar_usuario(id:string) {
    this.router.navigate(['/admin/usuario/editar', encryptNumber(id)]);
  }

  listaUsuarios(estado:string) {
    if(estado == "1"){
      this.lblTituloUsuario = "Habilitados";
    }else{
    ...
    this.http.post(this.url+"usuario/listaUsuarioByEmpresaByEstado", parametros).subscribe((datos_recibidos:any) => {
      Swal.close();
