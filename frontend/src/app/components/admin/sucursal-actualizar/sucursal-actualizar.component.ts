...
export class SucursalActualizarComponent implements OnInit, AfterViewInit {
  ...
  clickNavegarHaciaSucursales(){
    this.route.navigate(['admin/sucursal']);
  }
  ...
  actualizarDatosSucursal(){
    ...
    {
      let parametros = {
        'nombre': this.nombre_sucursal, 'sitio_web': this.sitioweb_sucursal, 'id_departamento': this.id_municipio,
        'zona': this.zona_sucursal, 'avenida': this.avenida_sucursal, 'calle': this.calle_sucursal,
        'numero': this.numero_casa_sucursal, 'referencia': this.referencia_sucursal, 'latitud': this.latitud_sucursal, 'longitud': this.longitud_sucursal,
        'telefono_uno': this.telefono_uno_sucursal, 'telefono_dos': this.telefono_dos_sucursal, 'telefono_tres': this.telefono_tres_sucursal,
        'id_empresa': localStorage.getItem('id_empresa'), 'id': this.id_Sucursal
      };
  
      this.http.post(this.url + "sucursal/actualizarDatosSucursal", parametros).subscribe((datos_recibidos:any) => {
        Swal.close();
  ...
  actualizarImagenById(_idSucursal:string){
    ...
    this.http.post(
    this.url + "sucursal/actualizarImagenById/" + _idSucursal + "/" + localStorage.getItem('id_empresa'),
    ...
    this.http.post(this.url+"sucursal/SucursalById", parametros).subscribe((datos_recibidos:any) => {
      ...
      if(datosMysql === undefined){
        if(datosNodejs === undefined){
          ...
          this.telefono_tres_sucursal = datos[0].telefono_tres;
